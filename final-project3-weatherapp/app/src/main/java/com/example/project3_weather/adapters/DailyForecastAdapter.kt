...
class DailyForecastAdapter(val context: Context, private val weatherResponse: WeatherResponse) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.tvTime.text =
            OtherUtils.dateTime(weatherResponse.list[mPosition].dt, "North America","dd MMM")
        ...
        holder.tvLowestTemperature.text =
            floor(weatherResponse.list[mPosition].main.tempMin).toInt().toString() + " °F"
        holder.tvHighestTemperature.text =
            ceil(weatherResponse.list[mPosition].main.tempMax).toInt().toString() + " °F"
    }
